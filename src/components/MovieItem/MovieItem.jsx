...
export default class MovieItem extends Component {
  ...
  render() {
    const { title, overview, release_date, poster_path, textView, id, getSession, genreIds } = this.props
      ...
      <Row wrap={false} className="movieItem">
        ...
        <div className="movieItem__info">
          <div className="movieItem__head">
            ...
            <div
              className={
                this.state.valueRate <= 3
                  ? 'movieRate__circl_red'
                  : this.state.valueRate <= 5
                  ? 'movieRate__circl_or'
                  : this.state.valueRate <= 7
                  ? 'movieRate__circl_yl'
                  : this.state.valueRate > 7
                  ? 'movieRate__circl_gr'
                  : 'movieRate__circl_red'
          ...
          <GenreConsumer>
            {(genre) => {
              return <div className="movieItem__genre">{this.getGenre(genreIds, genre)}</div>
            }}
          </GenreConsumer>
